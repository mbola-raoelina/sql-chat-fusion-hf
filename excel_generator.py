...
# Streamlit import removed for Flask compatibility
import time
import os
import logging
  ...
  <soapenv:Body>
    ...
    def handle_sql_to_excel(self, sql_query: str) -> tuple[bytes, int]:
        ...
        if not sql_query or "SELECT" not in sql_query.upper():
            ...
            excel_logger.error("No valid SQL query provided.")
            return None, 0
        
        try:
            excel_logger.info("🔧 Generating Excel report...")
            ...
            if excel_data:
                ...
                if row_count == 0:
                    excel_logger.warning(f"⚠️ Query returned no data. An empty Excel file has been generated. (Time: {duration:.2f}s)")
                else:
                    excel_logger.info(f"✅ Excel generation completed successfully! Generated {row_count} rows of data. (Time: {duration:.2f}s)")
            ...
            if excel_data:
                end_time = time.time()
                duration = end_time - start_time
                excel_logger.warning(f"⚠️ Using fallback Excel generation. Copy the SQL query and execute manually. (Time: {duration:.2f}s)")
            ...
            end_time = time.time()
            duration = end_time - start_time
            excel_logger.error(f"Excel generation failed. (Time: {duration:.2f}s)")
            ...
            end_time = time.time()
            duration = end_time - start_time
            excel_logger.error(f"Excel generation failed: {str(e)} (Time: {duration:.2f}s)")
