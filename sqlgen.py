...
try:
    ...
    # LangChain is optional - not required for main pipeline
...
def setup_logging():
    ...
    try:
        ...
        logger.info(f"=== SQLGEN LOGGING INITIALIZED ===")
...
try:
    ...
    logger.warning(f"OpenAI client initialization failed: {e} - using Hugging Face models only")
    openai_client = None
...
def get_primary_table_candidates(user_query: str) -> List[str]:
    ...
    for term in financial_terms:
        if term in ['invoice', 'payment', 'receivable']:
            primary_candidates.extend([
                ...
                'AR_CASH_RECEIPTS_ALL', 'AR_TRX_SUMMARY', 'AR_TRX_BAL_SUMMARY',
                'AR_DETAILED_DISTRIBUTIONS_ALL', 'AR_INTERIM_CASH_RECEIPTS_ALL'
            ])
            ...
            primary_candidates.extend([
                ...
                'AR_CASH_RECEIPTS_ALL', 'AR_PAYMENT_SCHEDULES_ALL', 'AR_TRX_SUMMARY',
                'AR_CUSTOMER_BALANCE_ITF', 'HZ_CUSTOMER_PROFILES_F', 'AR_TRX_BAL_SUMMARY',
                'AR_DETAILED_DISTRIBUTIONS_ALL', 'AR_INTERIM_CASH_RECEIPTS_ALL'
            ])
        elif term in ['vendor']:
            primary_candidates.extend([
    ...
    if not primary_candidates:
        primary_candidates = [
            ...
            'AR_CASH_RECEIPTS_ALL', 'AR_TRX_SUMMARY', 'AR_TRX_BAL_SUMMARY',
            'AR_CUSTOMER_BALANCE_ITF', 'HZ_CUSTOMER_PROFILES_F', 'AR_DETAILED_DISTRIBUTIONS_ALL',
            'AR_INTERIM_CASH_RECEIPTS_ALL', 'AR_CONS_INV_ALL', 'AR_CONS_INV_TRX_ALL'
        ]
    ...
    if collection and original_query:
            ...
            try:
                ...
                if relevant_columns_results and relevant_columns_results['documents']:
                    for doc_text in relevant_columns_results['documents'][0]:
                        ...
                        # ENHANCED: Also extract columns from document text with multiple patterns
                        elif "COLUMN:" in doc_text.upper() and table_name.upper() in doc_text.upper():
                            # Pattern 1: "COLUMN:" prefix
                            for line in doc_text.splitlines():
                                if line.strip().upper().startswith("COLUMN:"):
                            ...
                            column_matches = re.findall(column_pattern, doc_text.upper())
                            ...
                            create_matches = re.findall(create_table_pattern, doc_text.upper())
                            for match in create_matches:
                                # Extract column names from the parentheses
                                col_defs = [col.strip().split()[0] for col in match.split(',') if col.strip()]
                                for col in col_defs:
                                    if col and not is_generic_column(col) and not col.startswith('CONSTRAINT'):
                                        tables[table_name]["columns"].append(col)
                            ...
                            table_matches = re.findall(table_contains_pattern, doc_text.upper())
        ...
        for table_name in list(tables.keys()):
            try:
                ...
                column_query = f"{original_query} columns in table {table_name}"
                column_results = retrieve_docs_semantic_pinecone(column_query, k=200)  # Keep original cost-effective limit
                
                # Also try a broader search specifically for this table's columns
                table_column_query = f"table {table_name} columns"
                table_column_results = retrieve_docs_semantic_pinecone(table_column_query, k=200)  # Keep original limit
                
                # Merge results from both queries
                all_column_docs = []
                if column_results.get("success") and column_results.get("docs"):
                    all_column_docs.extend(column_results["docs"])
                if table_column_results.get("success") and table_column_results.get("docs"):
                    all_column_docs.extend(table_column_results["docs"])
                
                # Process all column documents from both queries
                for d in all_column_docs:
                        doc_text = d.get("text", "")
                        meta = d.get("meta", {})
                        ...
                        # ENHANCED: Also extract columns from document text with multiple patterns
                        elif (meta.get("table", "").upper() == table_name.upper() and 
                              doc_text and "COLUMN:" in doc_text.upper()):
                            # Pattern 1: "COLUMN:" prefix
                            for line in doc_text.splitlines():
                                if line.strip().upper().startswith("COLUMN:"):
                                    col_name = line.split(":", 1)[1].strip()
                ...
                # If we still don't have enough columns, try a more comprehensive search
                if len(tables[table_name]["columns"]) < 5:  # If we have very few columns, try harder
                    logger.warning(f"Table {table_name} has only {len(tables[table_name]['columns'])} columns, doing comprehensive search")
                    
                    # Try searching for ALL documents related to this table
                    comprehensive_query = f"{table_name}"
                    comprehensive_results = retrieve_docs_semantic_pinecone(comprehensive_query, k=300)  # Reasonable limit for fallback
                    
                    if comprehensive_results.get("success") and comprehensive_results.get("docs"):
                        for d in comprehensive_results["docs"]:
                            doc_text = d.get("text", "")
                            meta = d.get("meta", {})
                            
                            # If this document mentions the table, extract any column information
                            if (meta.get("table", "").upper() == table_name.upper() or 
                                table_name.upper() in doc_text.upper()):
                                
                                # Extract columns from any document that mentions this table
                                for line in doc_text.splitlines():
                                    if line.strip().upper().startswith("COLUMN:"):
                                        col_name = line.split(":", 1)[1].strip()
                                        if col_name and not is_generic_column(col_name):
                                ...
                                # Also look for column patterns in the text
                                import re
                                column_pattern = r'COLUMN:\s*([A-Z_][A-Z0-9_]*)'
                                column_matches = re.findall(column_pattern, doc_text.upper())
                                for col in column_matches:
...
3. CRITICAL: Table Alias Rules:
   - ALWAYS define table aliases in FROM and JOIN clauses BEFORE using them
   - Use simple aliases: a, b, c (one letter each)
   - NEVER use undefined aliases - if you use 'b.COLUMN', you MUST have 'JOIN TABLE_NAME b'
   - Use the SAME alias you defined - don't mix up aliases!
...
8. CRITICAL: Column Selection Process:
   ...
   - NEVER invent column names based on query context

9. CRITICAL: For calculated values:
   - Analyze column descriptions to understand business logic
   - Look for columns that represent the data you need
   - Calculate derived values using existing columns
   ...
   - THINK STEP BY STEP: What columns represent the data you need? How can you calculate the desired result?
...
10. CRITICAL: Data type awareness:
    ...
    - For date filtering, use the actual date column from the related table
...
11. CRITICAL: If a column doesn't exist:
    ...
    - NEVER assume column names exist - check ALL_AVAILABLE_COLUMNS first

12. CRITICAL: Column Name Assumptions to AVOID:
    - NEVER assume column names exist based on query context
    - NEVER invent column names that "should" exist semantically
    - ALWAYS check ALL_AVAILABLE_COLUMNS for the exact column names that exist
    - Use column descriptions to understand what each column represents
...
def build_semantic_prompt(user_query: str, tables_summary: Dict[str, Any], available_tables: List[str] = None) -> str:
    ...
    for t, data in top_tables:
        ...
        if essential_columns:
            ...
            if all_columns:
                lines.append(f"ALL_AVAILABLE_COLUMNS: {', '.join([f'{t}.{col}' for col in all_columns[:30]])}")  # Show more columns
                if len(all_columns) > 30:
                    lines.append(f"... and {len(all_columns) - 30} more columns")
                lines.append("⚠️  CRITICAL: ONLY use columns from the ALL_AVAILABLE_COLUMNS list above!")
    ...
    if top_tables:
        ...
        lines.append("")
        lines.append("🚨 FINAL WARNING: NEVER ASSUME COLUMN NAMES EXIST!")
        lines.append("🚨 ALWAYS check ALL_AVAILABLE_COLUMNS for the exact column names that exist")
        lines.append("🚨 Use column descriptions to understand what each column represents")
        lines.append("🚨 NEVER invent column names based on query context - verify they exist first!")
        ...
        lines.append("- CRITICAL: Table Alias Definition Pattern:")
        lines.append("  * FROM TABLE_NAME a  (defines alias 'a' for TABLE_NAME)")
        lines.append("  * JOIN TABLE_NAME b ON a.COLUMN = b.COLUMN  (defines alias 'b' for TABLE_NAME)")
        lines.append("  * Then use a.COLUMN_NAME and b.COLUMN_NAME in SELECT/WHERE clauses")
        lines.append("- NEVER use 'b.COLUMN' without first having 'JOIN TABLE_NAME b' somewhere in your SQL!")
...
def extract_sql_from_response(response: str, schema_columns: Dict[str, List[str]] = None) -> str:
    ...
    if (response.startswith('-- ERROR:') or 
        ...
        logger.error(f"LLM returned error message instead of SQL. Full response: {response}")
        logger.error(f"This usually indicates: 1) OpenAI API key issues, 2) Rate limits, 3) Model access issues, or 4) Network problems")
...
def create_dynamic_table_alias_map(sql: str) -> Dict[str, str]:
    """
    Create a dynamic mapping of table aliases to actual table names.
    This function robustly parses SQL to extract table-alias relationships.
    
    Returns: Dict mapping alias -> actual_table_name
    Example: {'A': 'AP_INVOICES_ALL', 'B': 'AR_PAYMENT_SCHEDULES_ALL'}
    """
    import re
    ...
    try:
        # Strategy 1: Extract FROM clause table-alias mappings
        # Use a single comprehensive pattern that handles both cases
        from_pattern = r'FROM\s+([A-Z_][A-Z0-9_]*)\s+(?:AS\s+)?([A-Z_][A-Z0-9_]*)'
        from_matches = re.findall(from_pattern, sql_upper)
        
        for table_name, alias in from_matches:
            # Skip if alias looks like a table name (too long or has underscores)
            if len(alias) > 3 or '_' in alias:
                continue
            # Skip SQL keywords
            if alias.upper() in {'AND', 'OR', 'WHERE', 'SELECT', 'FROM', 'JOIN', 'LEFT', 'RIGHT', 'INNER', 'OUTER', 'ON', 'GROUP', 'BY', 'ORDER', 'HAVING', 'UNION', 'ALL', 'DISTINCT', 'AS', 'IN', 'EXISTS', 'BETWEEN', 'LIKE', 'IS', 'NULL', 'NOT', 'CASE', 'WHEN', 'THEN', 'ELSE', 'END', 'IF', 'WHILE', 'FOR', 'WITH', 'RECURSIVE', 'LIMIT', 'OFFSET', 'TOP'}:
                continue
            table_alias_map[alias.upper()] = table_name.upper()
            logger.debug(f"FROM mapping: {alias} -> {table_name}")
        
        # Strategy 2: Extract JOIN clause table-alias mappings
        # Use a single comprehensive pattern that handles all JOIN types
        join_pattern = r'(?:LEFT\s+|RIGHT\s+|INNER\s+|OUTER\s+)?JOIN\s+([A-Z_][A-Z0-9_]*)\s+(?:AS\s+)?([A-Z_][A-Z0-9_]*)'
        ...
        for table_name, alias in join_matches:
            ...
            if alias.upper() in {'AND', 'OR', 'WHERE', 'SELECT', 'FROM', 'JOIN', 'LEFT', 'RIGHT', 'INNER', 'OUTER', 'ON', 'GROUP', 'BY', 'ORDER', 'HAVING', 'UNION', 'ALL', 'DISTINCT', 'AS', 'IN', 'EXISTS', 'BETWEEN', 'LIKE', 'IS', 'NULL', 'NOT', 'CASE', 'WHEN', 'THEN', 'ELSE', 'END', 'IF', 'WHILE', 'FOR', 'WITH', 'RECURSIVE', 'LIMIT', 'OFFSET', 'TOP'}:
                continue
            table_alias_map[alias.upper()] = table_name.upper()
            logger.debug(f"JOIN mapping: {alias} -> {table_name}")
        
        # Strategy 3: Handle subqueries and CTEs
        # CRITICAL FIX: Exclude SQL keywords from being treated as aliases
        sql_keywords = {
            'AND', 'OR', 'WHERE', 'SELECT', 'FROM', 'JOIN', 'LEFT', 'RIGHT', 'INNER', 'OUTER',
            'ON', 'GROUP', 'BY', 'ORDER', 'HAVING', 'UNION', 'ALL', 'DISTINCT', 'AS', 'IN',
            'EXISTS', 'BETWEEN', 'LIKE', 'IS', 'NULL', 'NOT', 'CASE', 'WHEN', 'THEN', 'ELSE',
            'END', 'IF', 'WHILE', 'FOR', 'WITH', 'RECURSIVE', 'LIMIT', 'OFFSET', 'TOP'
        }
        
        # Pattern: ) alias or WITH alias AS (but exclude SQL keywords)
        subquery_patterns = [
            r'\)\s+([A-Z_][A-Z0-9_]*)',  # Subquery aliases like ") sub"
            r'WITH\s+([A-Z_][A-Z0-9_]*)\s+AS\s*\(',  # CTE aliases
        ]
        
        for pattern in subquery_patterns:
            matches = re.findall(pattern, sql_upper)
            for alias in matches:
                # CRITICAL FIX: Skip SQL keywords and only process actual aliases
                if (len(alias) <= 3 and '_' not in alias and 
                    alias.upper() not in sql_keywords and
                    alias.upper() not in table_alias_map):  # Don't overwrite existing mappings
                    table_alias_map[alias.upper()] = f"SUBQUERY_{alias.upper()}"
                    logger.debug(f"Subquery mapping: {alias} -> SUBQUERY_{alias}")
        
        logger.info(f"Created dynamic table alias map: {table_alias_map}")
        return table_alias_map
        
    except Exception as e:
        logger.warning(f"Error creating dynamic table alias map: {e}")
        return {}
...
def validate_sql_against_schema(sql: str, available_columns: Dict[str, List[str]]) -> Dict[str, Any]:
    ...
    # ENHANCED: Preserve function parameters while cleaning for column extraction
    # Instead of removing ALL string literals, we'll be more selective
    
    # Step 1: Temporarily protect function calls and their parameters
    # This preserves TO_DATE('2025-08-01', 'YYYY-MM-DD') structure
    protected_functions = []
    function_pattern = r'\b(TO_DATE|TO_CHAR|TO_NUMBER|CAST|CONVERT|DECODE|COALESCE|NVL|ROUND|TRUNC|SUBSTR|UPPER|LOWER|LENGTH|REPLACE|COUNT|SUM|AVG|MIN|MAX)\s*\([^)]+\)'
    
    def protect_function(match):
        protected_functions.append(match.group(0))
        return f"__PROTECTED_FUNC_{len(protected_functions)-1}__"
    
    # Protect all function calls first
    sql_for_validation = re.sub(function_pattern, protect_function, sql_for_validation, flags=re.IGNORECASE)
    
    # Step 2: Now safely remove remaining string literals (not in functions)
    ...
    # Step 3: Restore the protected function calls
    for i, func in enumerate(protected_functions):
        sql_for_validation = sql_for_validation.replace(f"__PROTECTED_FUNC_{i}__", func)
    ...
    # Find table aliases in FROM clause: TABLE_NAME alias (including single letter aliases)
    from_matches = re.findall(r'FROM\s+([A-Z_][A-Z0-9_]*)\s+([A-Z])', sql_for_validation)
    for table, alias in from_matches:
        table_aliases.add(alias)
    
    # Also check for longer aliases 
    from_matches_long = re.findall(r'FROM\s+([A-Z_][A-Z0-9_]*)\s+([A-Z_][A-Z0-9_]*)', sql_for_validation)
    for table, alias in from_matches_long:
        table_aliases.add(alias)
    
    # Find table aliases in JOIN clauses: JOIN TABLE_NAME alias (including single letter aliases)
    join_matches = re.findall(r'JOIN\s+([A-Z_][A-Z0-9_]*)\s+([A-Z])', sql_for_validation)
    for table, alias in join_matches:
        table_aliases.add(alias)
    
    # Also check for longer aliases in JOIN
    join_matches_long = re.findall(r'JOIN\s+([A-Z_][A-Z0-9_]*)\s+([A-Z_][A-Z0-9_]*)', sql_for_validation)
    for table, alias in join_matches_long:
        table_aliases.add(alias)
    ...
    # DYNAMIC FIX: Create table alias mapping using our new robust function
    table_alias_map = create_dynamic_table_alias_map(sql)
    logger.debug(f"Using dynamic table alias map for validation: {table_alias_map}")
    ...
    for col in sql_columns:
        ...
        if '.' in col:
            ...
            # DYNAMIC FIX: Use the robust table alias mapping
            actual_table = table_alias_map.get(table_alias.upper())
            
            if not actual_table:
                # CRITICAL: Check if table alias is undefined
                validation_result["ok"] = False
                validation_result["errors"].append(f"Undefined table alias '{table_alias}' used in column reference '{col}'. Available aliases: {list(table_alias_map.keys())}")
                continue
    ...
    # Additional check for invalid JOIN logic using dynamic table alias mapping
    ...
    for table1_alias, col1, table2_alias, col2 in join_matches:
        # Use dynamic table alias mapping to get actual table names
        actual_table1 = table_alias_map.get(table1_alias.upper(), table1_alias.upper())
        actual_table2 = table_alias_map.get(table2_alias.upper(), table2_alias.upper())
        
        # Check if these are valid columns in their respective tables
        table1_cols = available_columns.get(actual_table1, [])
        table2_cols = available_columns.get(actual_table2, [])
        ...
        if table1_cols and table2_cols:
            ...
            if not col1_found:
                validation_result["ok"] = False
                validation_result["errors"].append(f"Column '{col1}' not found in table '{actual_table1}' (alias '{table1_alias}')")
            if not col2_found:
                validation_result["ok"] = False
                validation_result["errors"].append(f"Column '{col2}' not found in table '{actual_table2}' (alias '{table2_alias}')")
...
def call_openai_api(system_prompt: str, user_prompt: str, model: str = OPENAI_MODEL) -> str:
    ...
    if not openai_client:
        logger.error("OpenAI client not available. This usually means:")
        logger.error("1. OPENAI_API_KEY environment variable is not set")
        logger.error("2. OpenAI client failed to initialize (check logs above)")
        logger.error("3. OpenAI package is not installed")
        logger.error("4. Proxy environment variables are interfering with OpenAI initialization")
    ...
    try:
        ...
        if "rate_limit" in str(e).lower():
            ...
            return "-- ERROR: OpenAI model access denied. Check model permissions."
        else:
...
# Global HF model cache
_hf_model_cache = {}

def call_huggingface_model(system_prompt: str, user_prompt: str, model: str = "distilgpt2") -> str:
    """
    Calls Hugging Face model for SQL generation.
    """
    logger.info(f"Calling Hugging Face model: {model}")
    
    if not HF_AVAILABLE:
        logger.error("Hugging Face transformers not available")
        return "-- ERROR: Hugging Face transformers not available. Install with: pip install transformers torch"
    
    try:
        # Use Hugging Face model with memory optimization strategies
        if model not in _hf_model_cache:
            logger.info(f"Loading Hugging Face model: {model}")
            
            # Check if this is a T5 model (needs text2text-generation pipeline)
            is_t5_model = "t5" in model.lower() or "gaussalgo" in model.lower()
            pipeline_type = "text2text-generation" if is_t5_model else "text-generation"
            
            logger.info(f"Using pipeline type: {pipeline_type} for model: {model}")
            
            # Try with quantization optimizations first for memory efficiency
            try:
                if is_t5_model:
                    # T5 models need different handling
                    _hf_model_cache[model] = pipeline(
                        pipeline_type,
                        model=model,
                        tokenizer=model,
                        max_length=512,
                        ...
                        model_kwargs={"load_in_8bit": True}  # 8-bit quantization for T5
                    )
                else:
                    # For GPT-style models, try 4-bit quantization first (maximum memory savings)
                    _hf_model_cache[model] = pipeline(
                        pipeline_type,
                        model=model,
                        tokenizer=model,
                        ...
                        device_map="auto",  # Let transformers handle device placement
                        torch_dtype="float16",  # Use half precision for memory efficiency
                        model_kwargs={"load_in_4bit": True}  # 4-bit quantization for maximum memory savings
                    )
                logger.info(f"Successfully loaded {model} with quantization")
                
            except Exception as quant_error:
                logger.warning(f"4-bit quantization failed: {quant_error}, trying 8-bit quantization")
                try:
                    if is_t5_model:
                        _hf_model_cache[model] = pipeline(
                            pipeline_type,
                    ...
                    logger.warning(f"8-bit quantization failed: {quant8_error}, trying without quantization")
                    try:
                        if is_t5_model:
                            _hf_model_cache[model] = pipeline(
                                pipeline_type,
                                ...
                                torch_dtype="float32"  # Use full precision for CPU compatibility
                            )
                        else:
                            _hf_model_cache[model] = pipeline(
                                pipeline_type,
                        ...
                        logger.info(f"Successfully loaded {model} without quantization (CPU mode)")
                        
                    except Exception as cpu_error:
                        logger.error(f"All loading methods failed: {cpu_error}")
                        return f"-- ERROR: Failed to load model {model}: {cpu_error}"
        
        # Format prompt based on model type
        if is_t5_model:
            # T5 models work better with simpler prompts
            full_prompt = f"Generate SQL for: {user_prompt}\nSchema: {system_prompt}"
        else:
            # GPT-style models can handle more complex prompts
            full_prompt = f"""Generate SQL query for: {user_prompt}
...
SQL:"""
        ...
        # Remove any extra text after the SQL query
        if "###" in sql_part:
            sql_part = sql_part.split("###")[0].strip()
        
        logger.info(f"Generated SQL from {model}: {sql_part[:100]}...")
        return sql_part
        
    except Exception as e:
        logger.exception("Hugging Face model call failed")
        return f"-- ERROR: Hugging Face model failed: {e}"
...
def generate_sql_from_text_semantic(user_query: str, chroma_k: int = 50, model: str = None) -> Dict[str,Any]:
    ...
    if model is None:
        if OPENAI_API_KEY and openai_client:
            model = "gpt-4o-mini"  # Fast and reliable - prioritize API models
            logger.info("Auto-selected OpenAI gpt-4o-mini for optimal performance")
        elif USE_HF_MODELS and HF_AVAILABLE:
            model = "defog/sqlcoder-7b-2"  # SQL-optimized model with quantization
            logger.info("Auto-selected SQLCoder-7B-2 with quantization for HF Spaces compatibility")
        elif PREFER_OLLAMA or not openai_client:
            model = "sqlcoder:15b"  # Best SQL model for local Ollama
            logger.info("Auto-selected Ollama sqlcoder:15b")
        else:
            model = "gpt-4o-mini"  # Fallback to API model for reliability
            logger.warning("Auto-selected OpenAI gpt-4o-mini (fallback)")
    ...
    docs = r["docs"]
    if not docs:
        error_msg = "No semantically relevant schema information found for this query."
        
        # Provide more specific error messages based on the retrieval result
        if not r.get("success", True):
            error_msg += f" Retrieval error: {r.get('error', 'Unknown error')}"
        elif os.getenv("PINECONE_API_KEY"):
            error_msg += " This may indicate that the Pinecone index is empty, the embedding model doesn't match the indexed data, or the schema documents are not properly formatted."
        else:
            error_msg += " ChromaDB is not available or the local schema database is empty."
    ...
    # 4) Call LLM (OpenAI, Ollama, or Hugging Face) with fallback mechanism
    if model.startswith("gpt-") or model.startswith("o1-"):
        # Use OpenAI for GPT models
        llm_response = call_openai_api(SYSTEM_PROMPT, prompt, model=model)
    elif model.startswith("defog/") or model.startswith("microsoft/") or model.startswith("huggingface/") or model == "distilgpt2":
        # Use Hugging Face for HF models (including distilgpt2)
        llm_response = call_huggingface_model(SYSTEM_PROMPT, prompt, model=model)
    elif model.startswith("sqlcoder:"):
        # Use Ollama for local models (sqlcoder:15b, etc.)
        llm_response = call_ollama_rest(SYSTEM_PROMPT, prompt, model=model)
    else:
        # Default to Hugging Face for unknown models (safer for HF Spaces)
        logger.warning(f"Unknown model '{model}', defaulting to Hugging Face")
        llm_response = call_huggingface_model(SYSTEM_PROMPT, prompt, model=model)
    ...
    if llm_sql and not llm_sql.startswith("-- ERROR:"):
        ...
        if not schema_validation["ok"]:
            ...
            # HALLUCINATION RETRY MECHANISM: Retry with enhanced prompt
            logger.info("Attempting hallucination correction with enhanced prompt...")
            
            # Create enhanced prompt with validation feedback
            validation_feedback = f"""
VALIDATION FAILED - Previous attempt had errors:
{'; '.join(schema_validation['errors'])}

SUGGESTIONS FOR CORRECTION:
{chr(10).join(schema_validation['suggestions']) if schema_validation['suggestions'] else 'No specific suggestions available'}

CRITICAL: You must use ONLY the columns listed in ALL_AVAILABLE_COLUMNS above.
Do NOT invent or assume column names exist. Check the exact column names provided.
"""
            
            enhanced_prompt_with_feedback = prompt + "\n\n" + validation_feedback
            
            # Retry with the same model first
            try:
                logger.info(f"Retrying with enhanced prompt using model: {model}")
                if model.startswith("gpt-") or model.startswith("o1-"):
                    retry_response = call_openai_api(SYSTEM_PROMPT, enhanced_prompt_with_feedback, model=model)
                elif model.startswith("defog/") or model.startswith("microsoft/") or model.startswith("huggingface/") or model == "distilgpt2":
                    retry_response = call_huggingface_model(SYSTEM_PROMPT, enhanced_prompt_with_feedback, model=model)
                else:
                    retry_response = call_ollama_rest(SYSTEM_PROMPT, enhanced_prompt_with_feedback, model=model, timeout=120)
                
                retry_sql = extract_sql_from_response(retry_response, schema_columns)
                
                # Validate the retry attempt
                if retry_sql and not retry_sql.startswith("-- ERROR:"):
                    retry_validation = validate_sql_against_schema(retry_sql, available_columns)
                    if retry_validation["ok"]:
                        logger.info("Hallucination correction successful on first retry!")
                        llm_sql = retry_sql
                    else:
                        logger.warning(f"Retry still has validation issues: {retry_validation['errors']}")
                        # Try one more time with even more explicit instructions
                        final_feedback = f"""
SECOND VALIDATION FAILED - Still has errors:
{'; '.join(retry_validation['errors'])}

FINAL ATTEMPT: You MUST use ONLY these exact column names from ALL_AVAILABLE_COLUMNS:
{', '.join([f'{t}.{c}' for t, cols in available_columns.items() for c in cols[:10]])}

Do NOT use any column names not in this list. If you need a column that doesn't exist, 
find the semantically closest one from the list above.
"""
                        final_prompt = prompt + "\n\n" + final_feedback
                        
                        if model.startswith("gpt-") or model.startswith("o1-"):
                            final_response = call_openai_api(SYSTEM_PROMPT, final_prompt, model=model)
                        elif model.startswith("defog/") or model.startswith("microsoft/") or model.startswith("huggingface/") or model == "distilgpt2":
                            final_response = call_huggingface_model(SYSTEM_PROMPT, final_prompt, model=model)
                        else:
                            final_response = call_ollama_rest(SYSTEM_PROMPT, final_prompt, model=model, timeout=120)
                        
                        final_sql = extract_sql_from_response(final_response, schema_columns)
                        final_validation = validate_sql_against_schema(final_sql, available_columns)
                        
                        if final_validation["ok"]:
                            logger.info("Hallucination correction successful on final retry!")
                            llm_sql = final_sql
                        else:
                            logger.error("All hallucination correction attempts failed")
                            error_msg = "; ".join(final_validation["errors"])
                            if final_validation["suggestions"]:
                                error_msg += "\n\nSUGGESTIONS:\n" + "\n".join(final_validation["suggestions"])
                            llm_sql = f"-- ERROR: {error_msg}"
                else:
                    logger.error("Retry attempt returned error instead of SQL")
                ...
                logger.error(f"Hallucination retry failed: {e}")
                error_msg = "; ".join(schema_validation["errors"])
                if schema_validation["suggestions"]:
                    error_msg += "\n\nSUGGESTIONS:\n" + "\n".join(schema_validation["suggestions"])
                llm_sql = f"-- ERROR: {error_msg}"
    ...
    if llm_sql and llm_sql.startswith("-- ERROR:") and ("timed out" in llm_sql or "failed" in llm_sql or "LLM returned error message instead of SQL" in llm_sql):
        ...
        for fallback_model in fallback_models:
            try:
                ...
                elif fallback_model.startswith("defog/") or fallback_model.startswith("microsoft/") or fallback_model.startswith("huggingface/") or fallback_model == "distilgpt2":
                    llm_response = call_huggingface_model(SYSTEM_PROMPT, enhanced_prompt, model=fallback_model)
                else:
...
class OracleSQLChain:
    ...
    def __init__(self, model_name: str = "gpt-4o-mini"):
        if not LANGCHAIN_AVAILABLE:
            raise ImportError("LangChain is optional and not required for the main pipeline")
...
if __name__ == "__main__":
    ...
    p.add_argument("--model", default="sqlcoder:15b" if PREFER_OLLAMA or not openai_client else "gpt-4o-mini")
    ...
    if args.method == "langchain":
        if not LANGCHAIN_AVAILABLE:
            print("ERROR: LangChain is optional and not required for the main pipeline")
            sys.exit(1)
        try:
